...
local function checkAllowedActions()
    local data = TriggerServerPromise(Utils.eventsPrefix .. ':checkAllowedActions')
    ...
    placeableObjects = data.placeableObjects
...
local function openActionsMenu()
    ...
    if(canPlaceAnyObject) then
        table.insert(elements, {label = getLocalizedText('actions:place_object'), value = "place_object"})
    end
    ...
    function(data, menu) 
        ...
        elseif(action == "place_object") then
            TriggerEvent(Utils.eventsPrefix .. ":actions:placeObject", placeableObjects)
        end
    end,
    function(data, menu)
        menu.close()
