...
CreateThread(function()
    for i = 1, #availableComponents, 1 do
        ESX.RegisterUsableItem(availableComponents[i], function(source)
            TriggerClientEvent('greenwood_attachments:getCurrentWeapon', source)
            currentItem[source] = availableComponents[i]
        end)
    end
end)

RegisterNetEvent('greenwood_attachments:returnWeapon', function(weaponName)
    if currentItem[source] ~= nil then
        local xPlayer = ESX.GetPlayerFromId(source)
        local action = xPlayer.addWeaponComponent(weaponName, currentItem[source])
        if action then
            xPlayer.removeInventoryItem(currentItem[source], 1)
        end
        currentItem[source] = nil
    end
end)

RegisterNetEvent('greenwood_attachments:unequip', function(weaponName)
    local xPlayer = ESX.GetPlayerFromId(source)
    for i = 1, #availableComponents, 1 do
        if xPlayer.hasWeaponComponent(weaponName, availableComponents[i]) then
            xPlayer.removeWeaponComponent(weaponName, availableComponents[i])
            xPlayer.addInventoryItem(availableComponents[i], 1)
        end
    end
end)
