if Config.Player then 
    if Config.PlayerWeight then 
        ESX.RegisterCommand('openinv', 'admin', function(xPlayer, args, showError)
            ...
            if not target then 
                xPlayer.triggerEvent("inventory:notify", "error", "Spieler existiert nicht")
                return 
            end
    
            if xPlayer.getIdentifier() == target.getIdentifier() then
                xPlayer.triggerEvent("inventory:notify", "error", "Sie können Ihr eigenes Inventar nicht einsehen")
                return
            end
        ...
        TriggerEvent('es:addGroupCommand', 'openinv', 'admin', function(source, args, user)
            ...
            local target = ESX.GetPlayerFromId(args.id)
            if not target then 
                TriggerClientEvent("inventory:notify", source, "error", "Spieler existiert nicht")
                return 
            end
