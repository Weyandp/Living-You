...
function getNameFromIdentifier(identifier, returnTable)
...
AddEventHandler('roadphone:sendDispatch', function(source, message, job, position, anonym)
    ...
    if xPlayer then
        ...
        for _, target in pairs(targets) do
            TriggerClientEvent("roadphone:service:receiveMessage:job", target.id, targetmessage, 0, false)
...
RegisterCommand(Config.TwitterVerifyCommand, function(source, args)
    ...
    MySQL.Async.execute("UPDATE `roadshop_twitter_accounts` SET `verify`= @verifyValue WHERE roadshop_twitter_accounts.username = @username", {
...
RegisterCommand(Config.InstaVerifyCommand, function(source, args)
    local src = source
    ...
    MySQL.Async.execute("UPDATE `roadshop_instagram_accounts` SET `verify`= @verifyValue WHERE roadshop_instagram_accounts.username = @username", {
        ['@verifyValue'] = verifyValue,
        ['@username'] = label
    }, function(result)
        if (result == 1) then
            local message = verifyValue == 1 and _U('instagram_verify') or _U('instagram_remove_verify')
            TriggerClientEvent('roadphone:sendOffNotification', src, message)
        else
            TriggerClientEvent('roadphone:sendOffNotification', src, _U('command_required_information'))
        end
...
ESX.RegisterServerCallback('roadphone:server:getEmployees', function(source, cb, society)

    local users = {}

    MySQL.Async.fetchAll("SELECT identifier, firstname, lastname, job_grade, phone_number FROM users WHERE job = @job", {
        ['@job'] = society
    }, function(result)
